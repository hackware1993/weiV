...
class WeiVJavaHelper {
    ...
    fun createConst(key: Key, buildCount: Int, build: IBuild): ConstWidget {
        return if (buildCount == 0) {
            weiV!!.addContainerRenderWidget(ConstWidget(key = key, childWidgets = ArrayList())) {
                build.build()
            }
        } else {
            weiV!!.addLeafRenderWidget(ConstWidget(key = key))
        }
    }
